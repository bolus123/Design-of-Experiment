...
logistic.reg.model <- function(Y, X, intercept = TRUE, alpha = 0.05, maxiter = 1000, tol = 1e-6){
    ...
    while(sum(abs(b.old - b.new)) > tol){
        ...
        eta.star <- eta + (Y - pi) / (pi * (1 - pi))
        
        b.old <- b.new
        b.new <- solve(t(X) %*% V %*% X) %*% t(X) %*% V %*% eta.star
    
    }

    var.b <- diag(solve(t(X) %*% V %*% X))
...
debug(logistic.reg.model)

Y <- APD[, 3]
X <- APD[, 1:2]

logistic.reg.model(Y, X)
